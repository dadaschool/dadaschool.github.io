...
(function () {
  ...
  function 계산표새로() {
    ...
    var r = { 시각: 시각, 기온: 기온 };
    var p4 = 모델4.예측(r), p2 = 모델2.예측(r), 참 = M.실제값(시각, 기온);
    $("tbP4").textContent = 소수(p4, 1) + " kWh";
    $("tbP2").textContent = 소수(p2, 1) + " kWh";
    $("tbTrue").textContent = 소수(참, 1) + " kWh";

    var res = $("tbRes");
    var 차2 = Math.abs(p2 - 참), 차4 = Math.abs(p4 - 참);
    res.className = "res " + (차4 < 0.4 ? "ok" : "no");
    res.innerHTML =
      "단서 4개 AI는 <b>" + 소수(차4, 1) + "kWh</b> 차이, " +
      "단서 2개 AI는 <b>" + 소수(차2, 1) + "kWh</b> 차이로 맞혔습니다." +
      (차2 > 차4 + 0.5
        ? " 같은 데이터인데 <b>단서를 어떻게 만들어 줬는지</b>만 다릅니다."
        : " 이 상황은 둘 다 비슷하게 맞힙니다. <b>③·④ 시나리오</b>로 시험해 보세요.");
  }
  ...
  function 시나리오그리기() {
    ...
    var 줄 = M.시나리오.map(function (s, i) {
      var r = { 시각: s.시각, 기온: s.기온 };
      var 참 = M.실제값(s.시각, s.기온), p2 = 모델2.예측(r), p4 = 모델4.예측(r);
      function 판정(v) {
      ...
      return "<tr><td class='left'>" + ["①", "②", "③", "④"][i] + " " + s.이름 +
             ...
             "<td>" + M.냉난방필요도(s.기온) + "</td><td>" + M.학교운영중(s.시각) + "</td>" +
             "<td><b>" + 소수(참, 1) + "</b></td><td>" + 판정(p2) + "</td><td>" + 판정(p4) + "</td>" +
             "<td class='left'>" + s.설명 + "</td></tr>";
    }).join("");

    $("tbScTable").innerHTML =
      "<thead><tr><th class='left'>시나리오</th><th>시각</th><th>기온</th>" +
      "<th>냉난방<br>필요도</th><th>학교<br>운영중</th><th>실제</th>" +
      "<th>단서 2개<br>AI</th><th>단서 4개<br>AI</th><th class='left'>실제로는</th></tr></thead>" +
      "<tbody>" + 줄 + "</tbody>" +
      "<tfoot><tr><td class='left' colspan='5'><b>300개 기록 전체의 평균 오차 (kWh)</b></td>" +
      "<td>–</td><td><b class='mark-no'>" + 소수(모델2.평균오차, 1) + "</b></td>" +
