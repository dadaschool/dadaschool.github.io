...
(function (g) {
  ...
  var lessonCodes = {};
  /* 🔴 Cloudflare KV 의 «목록» 은 방금 만들고·고치고·지운 것이 몇 초~1분 늦게 반영된다.
     그대로 그리면 지운 문제가 목록에 남아 있다가 다시 접속해야 사라진다(사용자 신고).
     그래서 이 화면에서 한 일을 2분 동안 기억해 두고 목록에 덧입힌다(지운 것은 빼고 · 만든 것은 넣고). */
  var localOps = {};
  function noteLocal(code, p, isNew) {
    var row = { code: String(code), title: String((p && p.t) || "").slice(0, 80),
      parts: ((p && p.parts) || []).length, lesson: Number(p && p.n) || 0 };
    if (isNew) row.at = Math.floor(Date.now() / 1000);
    localOps[String(code)] = { at: Date.now(), row: row };
  }
  function fetchCodes(cb) {
    window.Code.codes(function (list) {
      var now = Date.now(), seen = {}, out = [];
      Object.keys(localOps).forEach(function (k) { if (now - localOps[k].at > 120000) delete localOps[k]; });
      list.forEach(function (x) {
        var key = String(x.code), op = localOps[key];
        seen[key] = true;
        if (op && op.del) return;
        out.push(op && op.row ? Object.assign({}, x, op.row) : x);
      });
      Object.keys(localOps).forEach(function (k) {
        var op = localOps[k];
        if (!op.del && op.row && !seen[k]) out.push(Object.assign({ at: Math.floor(now / 1000) }, op.row));
      });
  ...
  function make6() {
    if (!check()) return;
    var made = probForCode();
    window.Code.makeCode(made, function (code, err) {
      if (!code) { alert("코드를 만들지 못했습니다.\n" + (err || "")); return; }
      noteLocal(code, made, true);
      editing = code;
  ...
  function save6() {
    ...
    var saved = probForCode();
    window.Code.saveCode(editing, saved, function (ok, err) {
      if (!ok) { alert("저장하지 못했습니다.\n" + (err || "")); return; }
      noteLocal(editing, saved, false);
  ...
  function bind() {
    ...
    $("mainWrap").addEventListener("click", function (ev) {
      ...
      if (d.edit6) edit6(d.edit6);
        ...
        window.Code.delCode(d.del6, function (ok, err) {
          if (!ok) { alert("지우지 못했습니다.\n" + (err || "")); return; }
          localOps[String(d.del6)] = { at: Date.now(), del: true };
