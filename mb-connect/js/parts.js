...
(function (g) {
  ...
  var PARTS = [
    ...
    {
      /* 🔴 **전원핀이 없다** — 공통 캐소드. 접지를 함께 쓰고 R·G·B 세 핀으로 색을 만든다.
         ⚠ **공식 문서가 서로 어긋난다**(2026-08-24 확인).
           · 본문 : "Common Anode RGB LED module"  ← 전원 공용이라는 뜻
           · 배선 : "R·G·B → P0·P1·P2 + 접지 하나"(선 4개)  ← 접지 공용
           · 예제 코드 : 값을 **올리면 밝아진다**  ← 접지 공용이어야 그렇게 된다
           둘 대 하나로 **접지 공용**을 택했다. `V` 로 바꿔 넣었다가 되돌린 것이다.
         ⚠ 실물 판에 찍힌 네 번째 핀 글자(`G`인지 `V`·`+`인지)를 보면 곧바로 끝난다.
           다르면 이 `pins` 의 마지막 줄만 `{ n: "V", role: "vcc" }` 로 고치면 된다. */
      id: "rgbled", group: "자주", name: "RGB LED", model: "KY-016",
      pcb: "#1a1c20", face: "rgb", power: "3V3", ext: "",
      note: "빨강·초록·파랑을 따로 조절해 색을 만든다. 공통 캐소드라 **전원선이 없고** " +
            "접지선 하나에 신호선 셋을 쓴다.",
      pins: [
        { n: "R", role: "sig", sub: "빨강" },
        { n: "G", role: "sig", sub: "초록" },
