# -*- coding: utf-8 -*-
"""
추상화와 알고리즘 · 로컬 서버 실행 도우미

- 파이썬 기본 모듈만 사용한다. 추가 설치나 인터넷 연결이 필요 없다.
- 이 파일이 있는 폴더(abstraction-algo)를 웹 루트로 삼아 서버를 켜고,
  브라우저에서 index.html(홈)을 자동으로 열어 준다.
- 8400번 포트가 이미 쓰이고 있으면 8401, 8402 … 순서로 빈 포트를 찾는다.
  (binary-converter 8000번대 · ai-class 8100번대 · EnergyKeeper 8200번대 ·
   mb-bluetooth 8300번대와 겹치지 않게 8400번대를 쓴다.)
...
def main():
    ...
    print("=" * 60)
    print("  추상화와 알고리즘 · 로컬 서버")
    print("=" * 60)
    print("  홈           : %s" % url)
    print("  1차시 실험실 : http://localhost:%d/lesson1.html" % port)
    print("  1차시 학습지 : http://localhost:%d/worksheet1.html" % port)
    print("  2차시 실험실 : http://localhost:%d/lesson2.html" % port)
    print("  2차시 학습지 : http://localhost:%d/worksheet2.html" % port)
