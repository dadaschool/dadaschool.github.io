...
(function (global) {
  ...
  var METHOD_INFO = {
    ...
    /* `dec10` 도 `methodOf` 가 돌려주는 값이 **아니다.**
       `2진수 → 4진수` 처럼 **10진수가 양쪽에 없는** 방향에서 「① 진수 변환」 을 골랐을 때 쓴다.
       (2026-08-22 사용자 지시 — 예전에는 그 방향에 «묶기» 하나뿐이었다.
        묶기는 2가 밑이라서 되는 **요령**이므로 「② 2진수 활용」 으로 옮기고,
        **원리**인 10진수 경유를 ① 로 두었다) */
    dec10: {
      name: "10진수를 거쳐 두 번에",
      how: "바로 바꾸는 셈이 없으므로 우리가 아는 10진수를 거칩니다 — " +
           "① 자리값을 곱해 더해 10진수를 구하고, ② 그 10진수를 목표 진수로 반복해 나눕니다."
    },
    /* `dec2` 는 `methodOf` 가 돌려주는 값이 **아니다.**
       「방법 고르기」의 ① 진수 변환 은 예전처럼 반복 나누기를 쓰고,
       이것은 **③ 2단계 처리**(10 → 2 → 4·8·16)를 골랐을 때만 쓰는 설명이다.
       (2026-08-19 사용자 지시 — ①② 는 그대로 두고 ③ 을 「두 방법 비교」에서 바꿨다) */
    dec2: {
      name: "2단계 처리 (2진수를 거쳐서)",
      how: "① 10진수를 2진수로 바꾸고(자리값 카드에서 빼기), " +
           "② 그 2진수를 오른쪽부터 묶습니다. 앞에서 배운 두 가지를 이어서 하는 것입니다."
    },
    same: {
      name: "같은 진수",
